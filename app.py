...
# Self-healing initialization pipeline
@st.cache_resource
def get_ml_system():
    try:
        classifier, regressor, preprocessor = load_ml_artifacts()
        hist_df, curr_df = load_datasets()
        # Test inference on sample to guarantee zero schema mismatch
        predict_project_risk(curr_df.head(2), classifier, regressor, preprocessor)
        metadata = load_metadata()
        return classifier, regressor, preprocessor, hist_df, curr_df, metadata
    except Exception as e:
        # Auto-heal: regenerate datasets, retrain models in current runtime, and load
        generate_and_save_data()
        train_and_evaluate()
        classifier, regressor, preprocessor = load_ml_artifacts()
        hist_df, curr_df = load_datasets()
...
classifier, regressor, preprocessor, hist_df, curr_df, metadata = get_ml_system()

# Store predictions in session state for instant UI responsiveness
if 'curr_pred' not in st.session_state or len(st.session_state.curr_pred) != len(curr_df):
